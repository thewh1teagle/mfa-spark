...
export PATH="$CUDA_HOME/bin:$PREFIX/bin:$KALDI_ROOT/tools/openfst/bin:$PATH"
for _mfa_bin_dir in "$KALDI_ROOT"/src/*bin "$KALDI_ROOT"/src/bin; do
  if [ -d "$_mfa_bin_dir" ]; then
    export PATH="$_mfa_bin_dir:$PATH"
  fi
done
export LD_LIBRARY_PATH="$PREFIX/lib:$KALDI_ROOT/src/lib:$KALDI_ROOT/tools/OpenBLAS/install/lib:$CUDA_HOME/lib64:${LD_LIBRARY_PATH:-}"
